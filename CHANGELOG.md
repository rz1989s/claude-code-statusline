...
### Changed
- **Claude Code v2.1.196 compatibility** (docs/version update): Verified the statusline against CC v2.1.196 (npm `latest`+`next`, published 2026-06-29; installed binary → 2.1.196). v2.1.196 is a 27-bullet feature+bugfix release — headlined by **organization default models** (admins set an org-wide default shown as "Org default"/"Role default" in the `/model` picker when the user hasn't chosen one; a model *selection* feature that resolves to existing model IDs — **not** a new `claude-*` ID or pricing tier, same class as the v2.1.175/176/187 model-allowlisting features), readable default session names, clickable file attachments in chat, and an **MCP security fix** (`claude mcp list`/`get` no longer spawn `.mcp.json` servers a repo self-approved via a committed `.claude/settings.json`; untrusted workspaces show `⏸ Pending approval`), plus the streaming idle-watchdog now on by default for all providers (opt out via the new `CLAUDE_ENABLE_STREAM_WATCHDOG=0` env var), a CC-own TUI per-frame render optimization, a `/code-review` ~25% token reduction, and ~16 other fixes (background-job transcript preservation, rate-limit-warning flicker + telemetry over-count, `claude agents` side-panel/status, MCP OAuth scope, `/context` 0-tokens-on-Bedrock, `/deep-research` verifier reporting). None of it touches the statusline-stdin surface: **no new/changed/removed JSON fields, no new models or pricing, no env-var or render changes**. The lone near-miss items are all non-applicable — the org-default-models feature is model *selection* (no new `claude-*` ID, nothing for `lib/cost/pricing.sh`); `CLAUDE_ENABLE_STREAM_WATCHDOG` is CC-process streaming config (**not** forwarded to the statusline the way `COLUMNS`/`LINES` have been since v2.1.153); the MCP-security bullet changes `claude mcp list`/`get` CLI-output behavior in untrusted workspaces (the statusline guards `execute_mcp_list()` inside CC sessions and parses transcripts + native data per the v2.22.0 4-component design), not the stdin schema; and the rate-limit-warning + `/context`-Bedrock fixes touch CC's own UI surfaces, not the stdin `rate_limits.*`/`context_window.*` fields. The GitHub release and raw `CHANGELOG.md` carry the full 27-bullet v2.1.196 entry verbatim-identical, and npm `dist-tags` confirm `{ stable: 2.1.185, latest: 2.1.196, next: 2.1.196 }`; the official docs changelog still caps at v2.1.195 (the familiar <24h docs-lag pattern — release published 29 Jun, one day before this check). Verified by a clean render against a v2.1.196 blob (Opus 4.8 + 1M + vim VISUAL + effort/thinking + workspace.repo + pr + native rate_limits), `CC:2.1.196`, all 9 lines, exit 0; local full suite 1127 ok / 42 environmental not-ok (1169 of 1171 executed, 0 critical-category failures, grep-verified). Supported range is now v2.1.6–v2.1.196.
