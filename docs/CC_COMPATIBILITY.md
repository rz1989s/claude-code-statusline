...
Per-version compatibility log for Claude Code releases from v2.1.77 through v2.1.160. The statusline uses **feature detection** (field existence), not version comparison, so it gracefully handles missing or unknown fields via the `get_json_field()` abstraction.
