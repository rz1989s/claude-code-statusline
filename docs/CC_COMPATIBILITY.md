...
Per-version compatibility log for Claude Code releases from v2.1.77 through v2.1.177. The statusline uses **feature detection** (field existence), not version comparison, so it gracefully handles missing or unknown fields via the `get_json_field()` abstraction.
...
## v2.1.177 (13 Jun 2026) — empty-changelog maintenance release, promoted to latest (docs-only, target)

npm `latest` AND `next` dist-tag (`npm view @anthropic-ai/claude-code dist-tags` → `{ stable: 2.1.153, latest: 2.1.177, next: 2.1.177 }`); installed CC binary = 2.1.177 (`claude --version`). npm publish 2026-06-13T01:04Z; GitHub release tag `v2.1.177` published 2026-06-13T01:25Z.

**The defining trait of this release: no changelog content anywhere.** Unlike every prior promoted-to-`latest` release in this log, v2.1.177 ships with an **empty GitHub release body** (literally `"\n"`), and **both the docs changelog (code.claude.com) and raw `CHANGELOG.md` have no v2.1.177 entry at all — they cap at v2.1.176**. The only commit associated with the release tag is `chore: Update CHANGELOG.md and feed.xml` — release-pipeline housekeeping, not a feature commit. Third-party trackers (claudeupdates.dev) likewise cap at v2.1.176, and a targeted WebSearch surfaced no v2.1.177-specific content.

This is a new shape in the npm-vs-changelog-lag family. It is **not** the v2.1.160 pattern (a `next`-only canary never promoted to `latest`): v2.1.177 *is* the promoted public `latest`+`next`. The closest analogue is v2.1.159 (a no-op "internal infrastructure improvements" maintenance release) — except v2.1.159 at least carried a one-line changelog, whereas v2.1.177's changelog text is entirely absent. Most likely a release-pipeline/infra publish whose human-written notes were never authored (or get backfilled later); *Wallahu a'lam* — worth a `dist-tags` re-check next cycle, but the binary itself is render-verified clean now.

**Binary verification.** A `strings` scan of the installed v2.1.177 native executable confirms every known statusline stdin field (`used_percentage`, `remaining_percentage`, `context_window_size`, `rate_limits`/`five_hour`/`seven_day`/`resets_at`, `exceeds_200k_tokens`, `total_input/output_tokens`, `transcript_path`, `output_style`, `pr.review_state`, `workspace.git_worktree`, etc.) is present and unchanged, with **no new stdin-schema field names and no new model IDs**. The field-shaped tokens that do surface (`enforcement_percentage`, `budget_tokens`, `daemon_age_ms`, …) are CC-internal OTEL/MCP/daemon state, not statusline stdin.

No new models or pricing — Fable 5 (`claude-fable-5`, $10/$50, v2.1.170) remains the newest model, already priced in `lib/cost/pricing.sh` since v2.26.0; Opus 4.8 (`claude-opus-4-8`, $5/$25) priced since v2.25.0; sibling Mythos 5 remains absent from the authoritative model catalog (still deferred).

**Zero new/changed/removed statusline-stdin JSON fields, zero new model IDs, zero pricing changes, zero new statusline env vars, zero render changes** → **PATCH** bump (v2.26.3 → **v2.26.4**, docs-only). **Statusline fully compatible through v2.1.177 via feature detection** — verified by a **full 9-line render** against the v2.1.177 JSON blob (`claude-opus-4-8` + 1M + `vim:VISUAL` + `effort` + `thinking` + `workspace.repo` + `pr` + native `rate_limits`): clean output — `🧠 Claude Opus 4.8`, `CC:2.1.177` picked up straight from stdin, `Ctx: 12%` (1M window), native rate_limits `24%/80% • 41%/86%` (zero network), all nine lines rendered (incl. prayer times + `📍 Loc: Southeast Asia` + `MCP:27/27`), exit 0 (8th consecutive hang-free render). Local full suite (`npm test`): **1087 ok / 45 not-ok** (1132 of 1134 executed, 2 pre-existing skips; real npm exit = 1 via the `gtimeout 420 npm test > log 2>&1; echo REALEXIT=$?` wrapper) — the known-environmental set plus **3 in-session `--monthly` SIGKILLs** (#504–506, exit 137 — the documented `--monthly` JSONL-scan/date-filter hang inside an active CC session, killed by the suite timeout; tolerant "Stats-or-no-data" assertions, not wrong output), all the rest the usual `create_mock_command` `mock_bin/` cascade through prayer/GPS/location/IP-geolocation/connectivity, `test_cache_integration.bats` XDG/git temp-dir + `migrate_legacy_cache` BW01, in-session MCP-connectivity skew — **grep-verified zero pricing/schema/render/responsive/rate_limit/emoji/fable failures**, none related to v2.1.177, which changes zero statusline code. CI (Ubuntu) is the authoritative gate. Manual test command now uses `version: "2.1.177"`.
